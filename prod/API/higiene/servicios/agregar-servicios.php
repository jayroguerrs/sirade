<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        try {
            ...
            if ($contador == 0) {
                $stmt = $conn->prepare("SELECT NUSUA_ID FROM SRD_HIG_AREAS_SUPER WHERE CAREA_ID = ? AND NASUP_ESTADO = 1 AND NAUDI_EST_REG = 1;");
                $stmt->bind_param("s", $V_SERV);
                $stmt->execute();
                $stmt->store_result();
                if ($stmt->num_rows > 0) {
                    ...
                    $stmt = $conn->prepare("SELECT NUSUA_ID FROM SRD_HIG_AREAS_SUPER WHERE CAREA_ID = ? AND NASUP_ESTADO = ? AND NAUDI_EST_REG = 1;");
                    ...
                    if ($stmt->num_rows > 0) {
                        ...
                        $stmt = $conn->prepare("INSERT INTO SRD_HIG_AREAS_SUPER (CAREA_ID, NUSUA_ID, NAUDI_REG_INS)
                                                VALUES(?, ?, ?);");
