    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        try {
            ...
            if ( $V_CODIGO === NULL || $V_CODIGO == '' ) {
                ...
                $stmt = $conn->prepare("SELECT CUSUA_CODIGO FROM SRD_USUARIOS WHERE CUSUA_CODIGO = ? AND NAUDI_EST_REG = 1;");
                $stmt->bind_param("s", $V_CODIGO);
                $stmt->execute();
                $stmt->store_result();
                if ($stmt->num_rows > 0) {
            ...
            if ($contador == 0) {
                ...
                $stmt = $conn->prepare("INSERT INTO SRD_USUARIOS (
                                            CUSUA_CODIGO, CUSUA_DOCUMENTO, CUSUA_NOMBRES, CUSUA_IMG, NDESE_ID, NOCUP_ID, NNACI_ID, CUSUA_CORREO, NROLE_ID, NAUDI_REG_INS
                                        ) 
                                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);");
                                                
                $stmt->bind_param("ssssiiissi", $V_CODIGO, $V_DOCU, $V_NOMBRES, $V_IMAGEN, $V_DESE, $V_OCUP, $V_NACI, $V_CORREO, $V_ROLCOLAB, $V_ID);
                $stmt->execute();
