    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        try {
            ...
            if ($contador == 0) {
                ...
                $stmt = $conn->prepare("UPDATE SRD_JCI_CATEGORIA_PREG
                                        SET CJCAP_NOMBRE = ?, NJCAP_ORDEN = ?, NJCAP_ESTADO = ?, DAUDI_REG_UPD = CURRENT_TIMESTAMP(), NAUDI_REG_UPD = ?
                                        WHERE NJCAP_ID = ? ;");
                                                
                $stmt->bind_param('siiii', $V_CATE, $V_ORDEN, $V_ESTADO, $V_ID, $V_KEY);
                $stmt->execute();
