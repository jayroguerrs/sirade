<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $query = "SELECT 
                    ...
                    D.CDESE_DESCRIPCION
                FROM SRD_USUARIOS A
                LEFT JOIN SRD_HORARIO B ON A.NUSUA_ID = B.NUSUA_ID AND B.NHORA_ESTADO = 1 AND B.NAUDI_EST_REG = 1 
                LEFT JOIN SRD_SUPER_AREAS_PERIODO F ON F.NSAPE_ID = B.NSAPE_ID AND F.NSAPE_ESTADO = 1 AND F.NAUDI_EST_REG = 1 ";
        
        //Inicio Filtros de Busqueda personalizados
        if (!empty($V_PERI) && isset($V_PERI)) {
            $query .= " AND F.NPERI_ID = '" . $V_PERI . "' ";
        }
        ...
        $query .= " LEFT JOIN SRD_NACIONALIDAD C ON C.NNACI_ID = A.NNACI_ID
                    LEFT JOIN SRD_DESEMPENIO D ON D.NDESE_ID = A.NDESE_ID 
                    WHERE A.NUSUA_ESTADO = 1 AND A.NAUDI_EST_REG = 1 ";

        //Inicio Filtros de Busqueda personalizados
        if (!empty($V_SERV) && isset($V_SERV)) {
            $query .= " AND F.CAREA_ID = '" . $V_SERV . "' ";
        } else {
            $query .= " AND F.CAREA_ID IS NULL ";
        }
        ...
        function get_all_data($connect, $V_PERI, $V_SERV)
        {
            ...
            $query .= " WHERE A.NUSUA_ESTADO = 1 AND A.NAUDI_EST_REG = 1 ";
