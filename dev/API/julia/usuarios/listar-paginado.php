        ...
        try {
            ...
            if ($contador == 0) {
                ...
                $query = "  SELECT
                                ...
                                A.CUSUA_CORREO,
                                A.CUSUA_IMG,
                                GROUP_CONCAT(E.CROLE_NOMBRE SEPARATOR ', ') AS CROLE_NOMBRE,
                ...
                if (isset($_POST["search"]["value"])) {
                    ...
                    $query .= 'OR A.CUSUA_DOCUMENTO LIKE "%' . $_POST["search"]["value"] . '%" ';
                    $query .= 'OR A.CUSUA_NOMBRES LIKE "%' . $_POST["search"]["value"] . '%" ';
                    $query .= 'OR C.COCUP_DESCRIPCION LIKE "%' . $_POST["search"]["value"] . '%" ';
                    $query .= 'OR D.CNACI_DESCRIPCION LIKE "%' . $_POST["search"]["value"] . '%" ';
                    $query .= 'OR Z.CCADE_NOMBRE LIKE "%' . $_POST["search"]["value"] . '%") ';
                }
                
                $query .= " GROUP BY A.NUSUA_ID, Z.CCADE_NOMBRE, Y.CUSUA_CODIGO, Y.CUSUA_NOMBRES ";

                if (isset($_POST["order"])) {
                    $query .= 'ORDER BY ' . $_POST['columns'][$_POST['order']['0']['column']]['name'] . ' ' . $_POST['order']['0']['dir'] . ' ';
                } else {
                    $query .= 'ORDER BY FEC_MODIFICACION DESC ';
                }
                $query1 = '';
                if ($_POST["length"] != -1) {
                ...
                while($row = $result->fetch_assoc()) {
                    ...
                    $sub_array[] = $row["COCUP_DESCRIPCION"];                                               //[2]
                    $sub_array[] = $row["CNACI_DESCRIPCION"];                                               //[3]
                    $sub_array[] = $row["CNACI_IMAGEN"];                                                    //[4]
                    $sub_array[] = $row["CUSUA_CORREO"];                                                    //[5]
                    $sub_array[] = $row["CUSUA_IMG"];                                                       //[6]       
                    $sub_array[] = $row["CROLE_NOMBRE"];                                                    //[7]
                    $sub_array[] = $row["ESTADO"];                                                          //[8]
                    $sub_array[] = date_format(date_create($row["FEC_MODIFICACION"]), "d/m/Y h:i:s A");     //[9]
                    $sub_array[] = $row["USR_MODIFICACION"];                                                //[10]
                    $data[] = $sub_array;
                }
                
                function get_all_data($connect) {
                    
                    $query2 = "SELECT COUNT(A.NUSUA_ID) AS TOTAL
                                FROM SRD_USUARIOS A 
