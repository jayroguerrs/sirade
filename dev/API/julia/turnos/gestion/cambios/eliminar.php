    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        try {
            ...
            if ( $V_KEY === NULL || $V_KEY == '' ) {
                ...
                $stmt = $conn->prepare("SELECT NUSUA_ID FROM SRD_CAMBIOS_TURNO WHERE NCATU_ID = ? AND NAUDI_EST_REG = 1;");
            ...
            if ($contador == 0) {
                ...
                $stmt = $conn->prepare("UPDATE SRD_CAMBIOS_TURNO 
                                        SET NAUDI_EST_REG = ?, DAUDI_REG_UPD = NOW(), NAUDI_REG_UPD = ?
                                        WHERE NCATU_ID = ?;");
                                                
                $stmt->bind_param("iii", $V_ESTADO, $V_ID, $V_KEY);
                $stmt->execute();
