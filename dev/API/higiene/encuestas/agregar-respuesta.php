        ...
        try {
            ...
            if ( $V_ENCUE === NULL || $V_ENCUE == '' ) {
                ...
                $stmt = $conn->prepare("SELECT DISTINCT 
                                        ...
                                        WHERE B.NUSUA_ID = ? AND A.NHPER_ID = ? AND A.CAREA_ID = ? AND A.NHENC_ID = ? AND
                                            A.NHENC_ESTADO = 1 AND A.NAUDI_EST_REG = 1;");
            ...
            foreach ($_POST as $name => $value) {
                if (preg_match('/^radio_preg_(\d+)$/', $name, $matches)) {
                    ...
                    $pregunta = $matches[1];
                    $stmt = $conn->prepare("SELECT NJPRE_ID FROM SRD_HIG_PREGUNTAS WHERE NJPRE_ID = ? AND NJPRE_ESTADO = 1 AND NAUDI_EST_REG = 1;");
            ...
            if ($contador == 0) {
                ...
                foreach ($_POST as $name => $value) {
                    if (preg_match('/^radio_preg_(\d+)$/', $name, $matches)) {
                        ...
                        $stmt = $conn->prepare("SELECT NJRESP_ID FROM SRD_HIG_ENCUESTAS_PREG WHERE NHENC_ID = ? AND NJPRE_ID = ?");
                        ...
                        if ($stmt->num_rows > 0) {
                            ...
                            if ($respuestaActual != $value) {
                                ...
                                $stmt = $conn->prepare("UPDATE SRD_HIG_ENCUESTAS_PREG SET NJRESP_ID = ?, NAUDI_REG_UPD = ?, DAUDI_REG_UPD = CURRENT_TIMESTAMP() WHERE NHENC_ID = ? AND NJPRE_ID = ?");
                                $stmt->bind_param("iiii", $value, $V_SUPER, $V_ENCUE, $pregunta);
                                $stmt->execute();
                            }
                        } else {
                            // La pregunta no existe en la tabla, inserta un nuevo registro
                            $stmt = $conn->prepare("INSERT INTO SRD_HIG_ENCUESTAS_PREG (NHENC_ID, NJPRE_ID, NJRESP_ID, NHENC_SUPERVISOR, NAUDI_REG_INS) 
                                                    VALUES (?, ?, ?, ?, ?);");
                            $stmt->bind_param("iiiii", $V_ENCUE, $pregunta, $value, $V_SUPER, $V_SUPER);
                            $stmt->execute();
                        }
