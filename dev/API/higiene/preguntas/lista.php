        ...
        try {
            ...
            if ($contador == 0) {
                ...
                $query = " SELECT
                                ...
                                B.CHCAP_NOMBRE,
                                A.NHPRE_ORDEN,
                                FN_OBTENER_NOMBRE_ESTADO(A.NHPRE_ESTADO) ESTADO,
                            ...
                            INNER JOIN SRD_HIG_CATEGORIA_PREG B ON A.NHCAP_ID = B.NHCAP_ID
                            WHERE ";
                
                //Filtros de Busqueda personalizados
                if (!empty($V_PREG) && isset($V_PREG)) {
                    $query .= "A.CHPRE_DESCRIPCION LIKE %'" . $V_PREG . "'% AND ";
                }

                if (!empty($V_CATE) && isset($V_CATE)) {
                    $query .= "A.NHCAP_ID = '" . $V_CATE . "' AND ";
                }
                
                if (!empty($V_ESTA) && isset($V_ESTA) || $V_ESTA == '0') {
                    $query .= "A.NHPRE_ESTADO = " . $V_ESTA . " AND ";
                }
                ...
                if (isset($_POST["search"]["value"])) {
                    $query .= '(A.CHPRE_DESCRIPCION LIKE "%' . $_POST["search"]["value"] . '%" ';    
                    $query .= 'OR B.CHCAP_NOMBRE LIKE "%' . $_POST["search"]["value"] . '%" ';    
                    $query .= 'OR FN_OBTENER_NOMBRE_ESTADO(A.NHPRE_ESTADO) LIKE "%' . $_POST["search"]["value"] . '%") ';
                }
                ...
                if (isset($_POST["order"])) {
                    ...
                    $query .= 'ORDER BY A.NHPRE_ORDEN ASC ';
                }
                $query1 = '';
                if ($_POST["length"] != -1) {
                ...
                while($row = $result->fetch_assoc()) {
                    $sub_array = array();
                    $sub_array[] = $row["NHPRE_ID"];                                                    //[0]
                    $sub_array[] = $row["NHPRE_ORDEN"];                                                 //[1]
                    $sub_array[] = $row["CHPRE_DESCRIPCION"];                                           //[2]
                    $sub_array[] = $row["CHCAP_NOMBRE"];                                                //[3]
                ...
                function get_all_data($connect) {
                    ...
                    $query2 = "SELECT COUNT(DISTINCT A.NHPRE_ID) AS TOTAL
                                FROM SRD_HIG_PREGUNTAS A 
