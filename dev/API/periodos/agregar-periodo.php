    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        try {
            ...
            if ( $V_PERI === NULL || $V_PERI == '' ) {
                ...
                $stmt = $conn->prepare("SELECT NPERI_ID FROM SRD_PERIODO WHERE CPERI_DESCRIPCION = ? AND NAUDI_EST_REG = 1;");
            ...
            if ($contador == 0) {
                ...
                $stmt = $conn->prepare("INSERT INTO SRD_PERIODO (CPERI_DESCRIPCION, CAPLI_ID, DPERI_INICIO, DPERI_FIN, NAUDI_REG_INS) 
                                        VALUES (UCASE(?), ?, ?, ?, ?);");
                                                
                $stmt->bind_param("ssssi", $V_PERI, $V_APP, $fecha_inicio, $fecha_fin, $V_ID);
                $stmt->execute();
