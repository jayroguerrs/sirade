...
var KTPersonalList = function() {
    ...
    // Al seleccionar un item del select "periodo" debe listar los datos del select2 "desempenio"
    $("[name='periodo']").on('change.select2', function() {
        ...
        fetch(`${environment.apiSRD}/API/julia/supervisores/combo-servicios?${queryString}`, {
            method: 'GET'
        }).then(Response => Response.json())
        .then(datos => {
            if (datos.estado == 1) {
                var desempenioSelect = $("[name='servicio']");
                // Limpia el select "desempeño"
                desempenioSelect.empty();
        
                // Añade los nuevos datos al select "desempeño"
                datos.data.forEach(function(item) {
                    desempenioSelect.append('<option value="' + item.idArea + '">' + item.nombreArea + '</option>');
                });
        
                // Actualiza el select2
                desempenioSelect.trigger('change.select2');
            }
        })
    });
    ...
    var initDatatable1 = function() {
        ...
        dt1 = $("#colab_disponibles").DataTable({
            ...
            ajax: {
                type: "POST",
                url: `${environment.apiSRD}/API/julia/personal/lista-disponible`,
                data: function(data){
            ...
            columnDefs: [
                ...
                { targets: -1, visible: true,                   //Acciones
                    ...
                    render: function(data, type, row) {
                        return `
                        <a onclick="asignar('${(row[0])}', '${($("[name='periodo']").val())}');" class="btn btn-light btn-active-light-primary btn-sm" data-kt-menu-trigger="click" data-kt-menu-placement="bottom-end" data-kt-menu-flip="top-end">                                
    ...
    var initDatatable2 = function() {
        ...
        dt2 = $("#colab_asignados").DataTable({
            ...
            ajax: {
                type: "POST",
                url: `${environment.apiSRD}/API/julia/personal/lista-asignado`,
                data: function(data){
            ...
            columnDefs: [
                { targets: 0, visible: true,                   // Key
                    ...
                    render: function(data, type, row) {
                        return `
                      <a href="javascript:quitar('${(row[0])}');" class="btn btn-light btn-active-light-primary btn-sm" data-kt-menu-trigger="click" data-kt-menu-placement="bottom-end" data-kt-menu-flip="top-end">                                
...
function asignar(colaborador, periodo) {
    ...
    fetch(`${environment.apiSRD}/API/julia/personal/agregar-asignacion`, {
        method: 'POST',
        body: formData,
    }).then(Response => Response.json())
    .then(datos => {
...
function quitar(horario) {
    ...
    fetch(`${environment.apiSRD}/API/julia/personal/quitar-asignacion`, {
        method: 'POST',
        body: formData,
    }).then(Response => Response.json())
    .then(datos => {
