...
var KTAlumniList = function() {
    ...
    var initDatatable = function() {
        dt = $("#tb_alumni").DataTable({
            ...
            ajax: {
                type: "POST",
                url: "page/alumni/lista/lista-alumni.model.php",
                /*data: function(d){
            ...
            columnDefs: [{
                ...
                { targets: 1, visible: false }, //Paterno
                { targets: 2, visible: false }, //Materno
                { targets: 3, visible: false }, //Nombres
                {
                    targets: 4,
                    visible: true, //Nombre Compuesto
                    orderable: true,
                    ...
                    render: function(data, type, row) {
                        ...
                        if (user_img != '') {
                            // For Avatar image
                            var $output = '<div class=symbol-label><img src="/img/avatars/ alt="Avatar" class="w-100"></div>';
                        } else {
                            // For Avatar badge
                ...
                { targets: 5, visible: true }, //Código PUCP                  
                {
                    targets: 6,
                    visible: true, //Género
                    render: function(data, type, row) {
                        var class_genero = (data == 'M') ? 'primary' : ((data == 'F') ? 'warning' : 'success');
                        var pl_genero = (data == 'M') ? 'Masculino' : ((data == 'F') ? 'Femenino' : 'No Binario');
                        return `
                    <div class="badge badge-light-${class_genero}">${pl_genero}</div>
                  `
                    }
                },
                { targets: 7, visible: false }, //TipoDoc
                { targets: 8, visible: false }, //Numero Documento
                { targets: 9, visible: false }, //Fecha Nacimiento
                { targets: 10, visible: false }, //Estado Civil
                { targets: 11, visible: false }, //Pais Nacimiento
                { targets: 12, visible: false }, //Pais Residencia
                { targets: 13, visible: false }, //Departamento Residencia
                { targets: 14, visible: false }, //Email PUCP
                { targets: 15, visible: false }, //Email Personal
                { targets: 16, visible: false }, //LinkedIn
                { targets: 17, visible: false }, //Prefijo
                { targets: 18, visible: false }, //Número de Celular
                { targets: 19, visible: false }, //Carrera Pregrado
                { targets: 20, visible: false }, //Imagen
                {
                    targets: 21,
                    visible: true, //Nombre del Programa            
                    render: function(data, type, row) {
                        return `
                    <!--begin::User details-->
                    <div class="d-flex flex-column">
                      <a class="fs-7 text-gray-800 mb-1">${data}</a>
                      <span class="fs-8">${row[30]}</span>
                    </div>
                    <!--end::User details-->
                  `
                    }
                },
                { targets: 22, visible: false }, //Situacion Laboral
                { targets: 23, visible: false }, //Rubro
                { targets: 24, visible: false }, //Compañía
                { targets: 25, visible: false }, //Cargo Actual
                { targets: 26, visible: false }, //Cargo Estandar
                { targets: 27, visible: false }, //Labores de Docencia
                { targets: 28, visible: true }, //Año de Graduación
                { targets: 29, visible: false }, //Distinción
                { targets: 30, visible: false }, //Categoría
                { targets: 31, visible: false }, //Tipo de Postragrado
                {
                    targets: -1,
                    visible: true, //Acciones
                    data: null,
                    orderable: false,
                    className: 'text-end',
                    render: function(data, type, row) {
    ...
    var handleDeleteRows = () => {
        ...
        deleteButtons.forEach(d => {
            ...
            d.addEventListener('click', function(e) {
                ...
                Swal.fire({
                    text: "¿Está seguro que desea eliminar el registro: " + customerName + "?",
                    icon: "warning",
                    ...
                    if (result.value) {
                        ...
                        Swal.fire({
                            ...
                            Swal.fire({
                                text: "Se acaba de eliminar el registro: " + customerName + ".",
                                icon: "success",
                                buttonsStyling: false,
    ...
    var initToggleToolbar = function() {
        ...
        const container = document.querySelector('#tb_alumni');
